    ...
    <h1 class="mb-3">Modifier <?= $don['titre'] ?></h1>
      <form action="treatmentUpdateArtwork.php?id=<?= $don['id'] ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" id="titre" name="titre" class="form-control" placeholder="Titre du travail" value="<?= $don['titre'] ?>" required>
        </div>
        <div class="form-group mb-3">
            <label for="categorie">Catégorie</label>
            <select name="categorie" id="categorie" class="form-control">
                <?php
                    if($don['categorie']=="illustrations")
                    {
                        echo '<option value="illustrations" selected>Illustrations</option>';
                        echo '<option value="PAO">MiseEnPage</option>';
                        echo '<option value="dessinsVectoriels">DessinsVectoriel</option>';
                        echo '<option value="dessinsTattoo">DessinTattoo</option>';

                    } elseif($don['categorie']=="PAO")
                    {
                        echo '<option value="peinture">Peinture</option>';
                        echo '<option value="PAO" selected>MiseEnPage</option>';
                        echo '<option value="dessinsVectoriels">DessinsVectoriel</option>';
                        echo '<option value="dessinsTattoo">DessinTattoo</option>';

                    } elseif($don['categorie']=="dessinsVectoriels")
                    {
                        echo '<option value="peinture">Peinture</option>';
                        echo '<option value="PAO" >MiseEnPage</option>';
                        echo '<option value="dessinsVectoriels" selected>DessinsVectoriel</option>';
                        echo '<option value="dessinsTattoo">DessinTattoo</option>';

                    }
                    elseif($don['categorie']=="dessinsTattoo")
                    {
                        echo '<option value="peinture">Peinture</option>';
                        echo '<option value="PAO" >MiseEnPage</option>';
                        echo '<option value="dessinsVectoriels" >DessinsVectoriel</option>';
                        echo '<option value="dessinsTattoo" selected>DessinTattoo</option>';
