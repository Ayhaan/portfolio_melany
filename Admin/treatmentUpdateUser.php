...
if(!isset($_SESSION['login']))
{
 ...
 $req = $bdd->prepare("SELECT * FROM administration WHERE id=?");
 $req->execute([$id]);
...
if(isset($_POST['login']))
{
    ...
    if(empty($_POST['login']))
    {
        ...
        if($login!=$don['login'])
        {
            ...
            $reqLog = $bdd->prepare("SELECT * FROM administration WHERE login=?");
    ...
    if(empty($err))
    {
       $update = $bdd->prepare("UPDATE administration SET login=:log, email=:mail WHERE id=:myid");
       $update->execute([
           ":log"=>$login,
           ":mail"=>$email,
           ":myid"=>$id
